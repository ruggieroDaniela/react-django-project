...
export const FormForgotPassword = ({ message, info }) => {
    ...
    const handleSubmit  = async (e) =>  {
        ...
        if(info==="id"){
            try {
                const response = await axios.post(`${process.env.API_KEY}/forgot-password/`, {dni: input})
            ...
            try {
                const response = await axios.post(`${process.env.API_KEY}/forgot-password/`, {email: input})
                setUserMail(response.data.email) 
                setInvalidInput(false)
                setCorreoEnviado(true);
            } catch(error) {
