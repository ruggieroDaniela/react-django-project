...
export const FormForgotPassword = ({ message, info }) => {
    ...
    const handleSubmit  = async (e) =>  {
        ...
        if(info==="id"){
            try {
                const response = await axios.post(`${import.meta.env.DJANGO_API_URL}/forgot-password/`, {dni: input})
            ...
            try {
                const response = await axios.post(`${import.meta.env.DJANGO_API_URL}/forgot-password/`, {email: input})
                setUserMail(response.data.email) 
                setInvalidInput(false)
                setCorreoEnviado(true);
            } catch(error) {
