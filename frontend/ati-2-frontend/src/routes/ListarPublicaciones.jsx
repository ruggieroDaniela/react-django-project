    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                ...
                const response = await axios.get(`${process.env.API_KEY}/api-services/${postType}Service/${searchParams}`, {
                    headers: {}
                });
                            ...
                            <button
                                ...
                                onClick={ async () => {
                                    ...
                                    for (let i = 0; i < selectedPosts.length; i++) {
                                        await axios.put(`${process.env.API_KEY}/api-services/${postType}/enable_post/${selectedPosts[i].id}/`)
                                    }
                        ...
                        <li className="button" key={`${"self.crypto.randomUUID()"}`}>
                            <button
                                ...
                                onClick={ async () => {
                                    // eliminar 
                                    for (let i = 0; i < selectedPosts.length; i++) {
                                        await axios.put(`${process.env.API_KEY}/api-services/${postType}/delete_post/${selectedPosts[i].id}/`)
                                    }
                                    refreshPostList(prev => !prev);
                                } }
                            >
